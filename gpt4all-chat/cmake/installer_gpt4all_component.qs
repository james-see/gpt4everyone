        ...
        if (systemInfo.productType === "windows") {
            try {
                ...
                component.addOperation("CreateShortcut",
                    ...
                    "@UserProfile@/Desktop/GPT4Everyone.lnk",
                    "workingDirectory=" + targetDirectory + "/bin",
                    "iconPath=" + targetDirectory + "/gpt4all.ico",
                    "iconId=0", "description=Open GPT4Everyone");
            } catch (e) {
                print("ERROR: creating desktop shortcut" + e);
            }
            component.addOperation("CreateShortcut",
                targetDirectory + "/bin/chat.exe",
                "@StartMenuDir@/GPT4Everyone.lnk",
                "workingDirectory=" + targetDirectory + "/bin",
                "iconPath=" + targetDirectory + "/gpt4all.ico",
                "iconId=0", "description=Open GPT4Everyone");
        } else if (systemInfo.productType === "macos") {
            var gpt4everyoneAppPath = targetDirectory + "/bin/GPT4Everyone.app";
            var symlinkPath = targetDirectory + "/../GPT4Everyone.app";
            // Remove the symlink if it already exists
            component.addOperation("Execute", "rm", "-f", symlinkPath);
            // Create the symlink
            component.addOperation("Execute", "ln", "-s", gpt4everyoneAppPath, symlinkPath);
        } else { // linux
            var homeDir = installer.environmentVariable("HOME");
            if (!installer.fileExists(homeDir + "/Desktop/GPT4Everyone.desktop")) {
                component.addOperation("CreateDesktopEntry",
                    homeDir + "/Desktop/GPT4Everyone.desktop",
                    "Type=Application\nTerminal=false\nExec=\"" + targetDirectory +
                    "/bin/chat\"\nName=GPT4Everyone\nIcon=" + targetDirectory +
                    "/gpt4all-48.png\nName[en_US]=GPT4Everyone");
            }
        }
    } catch (e) {
...
Component.prototype.createOperationsForArchive = function(archive)
{
    ...
    if (systemInfo.productType === "macos") {
        ...
        var symlinkPath = uninstallTargetDirectory + "/../GPT4Everyone.app";
